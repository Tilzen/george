defmodule Greek.Leader do
  ...
  defmodule TraderData do
    defstruct pid: nil, reference: nil, state: nil
  end
  ...
  def handle_continue(:start_traders, %State{symbol: symbol} = state) do
  ...
  def handle_call(
    ...
    case Enum.find_index(traders, &(&1.pid == trader_pid)) do
      nil ->
        Logger.warn(
          "Tried to perform an update on a trader that the " <>
      ...
      index ->
...

        {
          :reply,
          :ok,
          %{state | :traders => List.replace_at(traders, index, new_trader_data)}
        }
    end
  end
  ...
  def handle_info(
    ...
    case Enum.find_index(traders, &(&1.pid == trader_pid)) do
      nil ->
        Logger.warn("Tried to remove finished trader that the leader does not know.")
        {:noreply, state}
  ...
  def handle_info(
    ...
    case Enum.find_index(traders, &(&1.pid == trader_pid)) do
      nil ->
        Logger.warn("Tried to restart trader but failed to find it's cached state.")
        {:noreply, state}
  ...
  defp start_new_trader(%Trader.State{} = state) do
    {:ok, pid} =
      DynamicSupervisor.start_child(
        :"Greek.DynamicSupervisor-#{state.symbol}",
        {Greek.Trader, state}
      )

    reference = Process.monitor(pid)

    %TraderData{pid: pid, reference: reference, state: state}
  end
